...
#define PV_EVLOG_MAX_ENTRIES  64
#define PV_EVLOG_TEXT_BYTES   96

typedef struct {
    uint32_t ms;                    // millis since boot
    char     text[PV_EVLOG_TEXT_BYTES];
} pv_evlog_entry_t;
...
// Append a printf-formatted line. Truncated to PV_EVLOG_TEXT_BYTES-1. Safe to
// call from any task once pv_evlog_init has been called; also safe to call
