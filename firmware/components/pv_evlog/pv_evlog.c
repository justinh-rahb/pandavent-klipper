...
static pv_evlog_entry_t  s_buf[PV_EVLOG_MAX_ENTRIES];
...
void pv_evlog_init(void)
{
    ...
    // pv_evlog_add becomes a no-op — good enough for a diagnostic aid.
}
...
void pv_evlog_add(const char *fmt, ...)
{
    ...
    pv_evlog_entry_t *e = &s_buf[s_head];
    ...
    s_head = (s_head + 1) % PV_EVLOG_MAX_ENTRIES;
    if (s_count < PV_EVLOG_MAX_ENTRIES) s_count++;

    xSemaphoreGive(s_lock);
}

size_t pv_evlog_snapshot(pv_evlog_entry_t *out, size_t max)
{
    ...
    size_t idx = (s_head == 0) ? (PV_EVLOG_MAX_ENTRIES - 1) : (s_head - 1);
    for (size_t i = 0; i < want; ++i) {
        out[i] = s_buf[idx];
        idx = (idx == 0) ? (PV_EVLOG_MAX_ENTRIES - 1) : (idx - 1);
    }
